...
import { hexCharCodeToAscStr, strSexteenSplit, strTwoSplit, isInteger, padNumber, formatFileSize } from '@/utils/tool'
import { apiPost } from '@/services/api';
const RadioGroup = Radio.Group;
function DataStream({ sessionId }) {
  ...
  const getSessionDataStream = async (value?: any) => {
    setLoading(true)
    const values: any = await apiPost('/api/getSessionDataStream', { sessionId: record.belongSessionId ?? record.sessionId })
    ...
    datas.map(item => {
      if (item.hexData !== '') {
        ...
        item.sigleHexData = arr.sigleHexData
      }
    })
    if (pageNum === 1) {
      setData(datas)
  ...
  const getAscTransformation = (hex) => {
    ...
    for (let i = 0; i < hexAscData.length; i++) {
      ...
      sigleHexData.push(offset)
    }
    return { hexData, ascData, sigleHexData, hexAscData }
  }
  useEffect(() => {
    ...
    //       if(countObj?.totalPacketCount === data.length) return
    //       setLoading(true)
    //       ++pageNum
    //       getSessionDataStream()
    //     }
  ...
  return <div>
    ...
    <Spin loading={loading} block>
        ...
        <div className='stream-main' ref={leftRef}>
          {data.map((item, i) => <div key={i} className={item.srcNode == countObj?.node0 ? 'send' : 'accept'}>
            {item.ascData.map((info, j) =>
              <pre key={j} style={{ marginBottom: 0, whiteSpace: 'break-spaces', fontFamily: 'Consolas, "Microsoft YaHei", Menlo, Monaco, "PingFang SC", "Hiragino Sans GB", "Heiti SC", sans-serif' }}>{info}</pre>
            )}
          </div>)}
        </div>}
      ...
      {type === 'hex' && <div className='stream-main'>
        {data.map((item, index) => <div className={`${item.srcNode == countObj?.node0 ? 'send' : 'accept'} stream-offset mgb20 hex-consolas`} key={index}>
          <div className="left">
            {item.sigleHexData.map((v) => <div key={v}>{v}</div>)}
          </div>
