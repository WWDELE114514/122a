...
namespace krabs { namespace details {
    ...
    private:
        ...
        EVENT_TRACE_LOGFILEW fill_logfile();
        void close_trace();
        void register_trace();
        EVENT_TRACE_PROPERTIES query_trace();
        void stop_trace();
        EVENT_TRACE_LOGFILEW open_trace();
        void process_trace();
     ...
     */
    template <typename T>
    static ULONG __stdcall trace_buffer_callback(EVENT_TRACE_LOGFILEW*pLogFile)
    {
    ...
    template <typename T>
    EVENT_TRACE_LOGFILEW trace_manager<T>::open()
    {
        register_trace();
        enable_providers();
        return open_trace();
    ...
    template <typename T>
    EVENT_TRACE_LOGFILEW trace_manager<T>::fill_logfile()
    {
        EVENT_TRACE_LOGFILEW file = {};
    ...
    void trace_manager<T>::register_trace()
    {
        ...
        ULONG status = StartTraceW(&trace_.registrationHandle_,
                                  trace_.name_.c_str(),
                                  &info.properties);
        if (status == ERROR_ALREADY_EXISTS) {
            try {
                stop_trace();
                status = StartTraceW(&trace_.registrationHandle_,
                    trace_.name_.c_str(),
                    &info.properties);
            }
    ...
    template <typename T>
    EVENT_TRACE_LOGFILEW trace_manager<T>::open_trace()
    {
        auto file = fill_logfile();
        trace_.sessionHandle_ = OpenTraceW(&file);
        if (trace_.sessionHandle_ == INVALID_PROCESSTRACE_HANDLE) {
